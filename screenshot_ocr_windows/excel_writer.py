...
# 模板路径（PyInstaller 打包时从 _MEIPASS 读取，开发环境用同目录）
import runtime_check
TEMPLATE_PATH = runtime_check.resource_path("模板.xlsx")

# 输出目录（导出 Excel 默认保存到桌面，打包模式无法写 EXE 内部）
OUTPUT_DIR = runtime_check.get_desktop_dir()
