...
# 必须在 import database 之前设置独立测试库。使用专属临时目录，避免复用
# /tmp 中可能被其他用户或旧进程留下的只读同名数据库。
TEST_DIR = None
TEST_DB = os.environ.get("OCR_LEARNING_DB")
if not TEST_DB:
    TEST_DIR = tempfile.mkdtemp(prefix="lq_price_check_test_")
    TEST_DB = os.path.join(TEST_DIR, "ocr_learning.db")
...
def test_import_rejects_summary_sheets_and_invalid_shop_names():
    print("== 投喂表头与内容校验 ==")
    wb_path = os.path.join(tempfile.gettempdir(), "feed_sheet_guard_windows.xlsx")
    from openpyxl import Workbook
...
def test_migrate_skips_invalid_legacy_names():
    print("== 旧库迁移校验 ==")
    legacy_path = os.path.join(tempfile.gettempdir(), "legacy_shop_city_guard_windows.db")
