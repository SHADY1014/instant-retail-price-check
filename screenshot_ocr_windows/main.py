...
class CityDetectWorker(QThread):
    ...
    def run(self):
        try:
            ...
            logger.exception("city_worker_failed")
    ...
    def _emit_finished(self, results=None, error=""):
        ...
        for signal, value, label in (
            ...
            try:
                ...
                logger.exception(
                    "region_city_result_emit_failed signal=%s", label)
...
class MainWindow(QWidget):
    ...
    def init_ui(self):
        ...
        self.store_check_btn.setToolTip(
            "仅把燕京U8巡查表转换为总部供货渠道价格明细（含城市列，共13列）"
        )
    ...
    def _on_cities_detected(self, shop_to_city):
        logger.info("city_detection_callback shops=%d", len(shop_to_city))
        try:
            self._on_cities_detected_impl(shop_to_city)
        except Exception:
            logger.exception("city_detection_callback_failed")
            QMessageBox.critical(
                self, "城市识别失败",
                "城市识别阶段发生异常。\n\n详细日志：\n" + runtime_check.get_log_path()
            )
    ...
    def _batch_set_shop_cities(self):
        ...
        logger.info("manual_batch_city_assignment shops=%d", len(unmatched_shops))
    ...
    def _prompt_batch_city(self, unmatched_shops):
        ...
        while remaining:
            ...
            hint = QLabel(
                f"请勾选属于「{city}」的店铺（剩余 {len(remaining)} 家待分配）："
            )
    ...
    def _populate_table(self):
        ...
        for path, fields in self.ocr_results.items():
            ...
            # 判断品牌
            brand_idx = detect_brand(d["product_name"])
            brand_names = {0: "燕京", 1: "雪花", 2: "青岛", 3: "百威"}
            brand_name = brand_names.get(brand_idx, "燕京")
            ...
            values = [
                ...
                d["shop_name"],
                d["product_name"],
                str(d["original_price"]) if d["original_price"] else "",
                str(d["final_price"]) if d["final_price"] else "",
                str(d["shop_discount"]) if d["shop_discount"] else "",
                str(d["full_reduction"]) if d["full_reduction"] else "",
                str(d["coupon"]) if d["coupon"] else "",
                str(d["red_packet"]) if d["red_packet"] else "",
                str(d["delivery_fee"]) if d["delivery_fee"] else "",
                brand_name,
                d["remark"],
            ]
    ...
    def _convert_store_check(self):
        ...
        self.status_label.setText("正在生成燕京U8总部供货渠道价格明细...")
        ...
        try:
            ...
            QMessageBox.information(
                self,
                "转换成功",
                f"已生成 {len(result.rows)} 行总部供货渠道价格明细：\n"
                f"{result.output_path}{pending_text}",
            )
            self.status_label.setText(
                f"总部供货渠道价格明细已生成：{os.path.basename(result.output_path)}"
            )
        except Exception as exc:
...
def main():
    log_path = runtime_check.configure_logging()
    runtime_check.install_exception_logging()
    logger.info("main_start log=%s", log_path)
    if not runtime_check.run_preflight():
        sys.exit(1)
