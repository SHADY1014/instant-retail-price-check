...
面向内部价格巡查的跨平台工具：将美团、淘宝和京东闪送/秒送结算截图识别为结构化记录，支持人工核对、城市确认、查重、Excel 巡查表导出，并可将巡查表转换为总部供货渠道价格明细。
...
总部供货渠道价格明细转换当前仅限燕京 U8：按片区归一化门店名，合并跨平台记录并取最低成交价，保留每行城市，重新计算“是否违规”；同时输出“转换日志”和“待确认清单”，原始巡查表不会被覆盖。
