"""Regression tests for the conservative macOS city decision flow."""
...
TEST_DIR = tempfile.mkdtemp(prefix="lq_macos_network_city_test_")
...
class NetworkCityDecisionTests(unittest.TestCase):
    ...
    def test_search_queries_keep_branch_name_before_fallbacks(self):
    ...
    def test_ambiguous_candidates_require_manual_review(self):
        ...
        self.assertEqual(
            {item.city for item in decision.candidates}, {"广州市", "深圳市"}
        )

    def test_weak_single_candidate_is_not_auto_filled(self):
        candidates = [
            city_detector.PoiCandidate(
