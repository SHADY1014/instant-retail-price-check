...
class FieldParser {
  ...
  static FormFields parse(List<OcrResult> ocrResults, {String region = ''}) {
    ...
    // 步骤 1: 平台识别
    // =========================================================
    final isTaobao = ['蜂鸟', '淘金币', '淘金市', '淘宝', '提交订单']
        .any((k) => fullText.contains(k));
    fields.platform = isTaobao ? '淘宝闪购' : '美团闪购';
    ...
    // 步骤 8: 优惠券
    // =========================================================
    fields.coupon = _findCoupon(lines);
  ...
  static const _shopKeywords = [
    '超市', '便利店', '店）', '店)', '店（', '店(', '送酒', '客超市', '商店',
    '嗨酒', '酒类', '酒行', '酒业', '酒零鹿', '鸡尾酒', '酒水', '酒栈', '酒屋',
  ];

  static const _nonShopTexts = [
    '送出', '送达', '去选择', '自配', '商品总价', '打包费', '配送费',
    '商家活动', '美团红包', '店铺券', '极速支付', '找人付', '共减',
    '收货人', '超时退费', '安心购', '赠',
  ];
    ...
    if (shopName.isEmpty && platform == '淘宝闪购') {
      for (final item in lines) {
        ...
        if (item.top > AppConstants.topTaobaoShop &&
          ...
          final m =
              RegExp(r'^(闪购|沟购|河购)\s*(.+)').firstMatch(text);
          if (m != null) {
    ...
    if (shopName.isEmpty && platform == '美团闪购') {
      for (final item in lines) {
        ...
        if (item.top > AppConstants.topMeituanShop &&
            ...
            item.left < AppConstants.shopLeftStrict) {
          // 排除非店铺文本
          if (['商品总价', '打包费', '配送费', '商家活动', '收货', '地址', '收货人',
                  '送出', '送达']
              .any((k) => text.contains(k))) {
            continue;
          }
  ...
  static String _findProductName(List<OcrResult> lines) {
    ...
    if (productTitle.isNotEmpty) {
      return ProductNormalizer.normalize(
        productTitle,
        specText,
        subTitle: subTitle,
            ...
            for (final item in lines) {
              if ((item.top - shipTop).abs() < AppConstants.alignNormal &&
                ...
                if (prices.isNotEmpty) {
                  var val = double.tryParse(prices.first.group(1)!)?.abs() ?? 0.0;
                  // 原价也可能漏小数点（¥69 实际 6.9）
  ...
  static double _findFinalPrice(List<OcrResult> lines, String platform) {
    ...
    if (!foundFinal && platform == '淘宝闪购') {
      for (final item in lines) {
        ...
        if (text.contains('合计') && item.top < AppConstants.topTaobaoTotal) {
          ...
          if (prices.isNotEmpty) {
            final price = PriceParser.extractPriceSafe('¥${prices.last.group(1)}');
            if (price > 0) {
              finalPrice = price;
              foundFinal = true;
              break;
    ...
    if (!foundFinal) {
      final bottomItems =
          lines.where((item) => item.top < AppConstants.topBottomPrice).toList();
      if (bottomItems.isNotEmpty) {
        ...
        if (m != null) {
          ...
          if (prices.isNotEmpty) {
            final price = PriceParser.extractPriceSafe('¥${prices.first.group(1)}');
            if (price > 0) {
              finalPrice = price;
              foundFinal = true;
            }
  ...
  static double _findShopDiscount(List<OcrResult> lines, String platform) {
    ...
    if (discountIdx < 0) {
      ...
      for (final item in lines) {
        if (item.left < AppConstants.shopLeftLoose && item.text.contains('活动')) {
    ...
    if (platform == '淘宝闪购' && discount == 0.0) {
      ...
      if (discIdx >= 0) {
        ...
        if (discount == 0.0) {
          for (final item in lines) {
            final text = item.text.trim();
            if (text.contains('已优惠') && item.top < AppConstants.topBottomRecommend) {
              // 格式如 "已优惠¥20.1 ¥70.2"
  ...
  static double _findFullReduction(List<OcrResult> lines, String platform) {
    ...
    if (mrIdx >= 0) {
      fullReduction = PriceParser.findPriceByXAlignment(lines, lines[mrIdx].top);
    }
  ...
  // =========================================================
  static double _findCoupon(List<OcrResult> lines) {
    double coupon = 0.0;
